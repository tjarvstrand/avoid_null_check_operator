...
for readme in README.md example/README.md; do
  if [ -f "$readme" ]; then
    sed -i.bak "s/avoid_null_check_operator: \^[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*/avoid_null_check_operator: $constraint/g" "$readme"
    rm -f "$readme.bak"
  fi
done
...
if [ "$vcs" = "jj" ]; then
  ...
  git add "$changelog" pubspec.yaml README.md example/README.md
